...
Процедура УстановитьOneScript(Знач ВерсияКУстановке, Знач АлиасВерсии = "") Экспорт
...
Функция СкачатьФайлУстановщика(Знач ВерсияКУстановке)
	...
	Соединение = Новый HTTPСоединение(ПараметрыOVM.ПутьКСайтуОСкрипт());
...
Процедура ПроверитьКорректностьПереданнойВерсии(Знач ВерсияКУстановке)
	Если СтрРазделить(ВерсияКУстановке, ".").Количество() <> ПараметрыOVM.КоличествоЦифрВНомереВерсии()
...
Функция ПолучитьПутьКСкачиваниюФайла(Знач ВерсияКУстановке)
	...
	Если СтрРазделить(ВерсияКУстановке, ".").Количество() = ПараметрыOVM.КоличествоЦифрВНомереВерсии() Тогда
