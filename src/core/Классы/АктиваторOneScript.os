...
Процедура СоздатьСимЛинкНаКаталог(Знач Ссылка, Знач ПутьНазначения)
	...
	// Если симлинк существует, но ведет на несуществующий путь - удалим его
	Если ФС.КаталогСуществует(Ссылка)
		И НЕ ФС.КаталогСуществует(ОбъединитьПути(Ссылка, "bin")) Тогда
