...
Функция ПолучитьСписокДоступныхКУстановкеВерсий() Экспорт
	...
	ПутьКСайтуОСкрипт = ПараметрыOVM.ПутьКСайтуОСкрипт();
	
	Соединение = Новый HTTPСоединение(ПутьКСайтуОСкрипт);
	...
	Для Каждого СовпадениеРегулярногоВыражения Из Совпадения Цикл
		...
		ДоступнаяВерсия.Путь = ПутьКСайтуОСкрипт + ГруппаАдрес.Значение;
	КонецЦикла;
...
Процедура ОбеспечитьСтрокуВерсииПоАлиасу(ТаблицаВерсий, Алиас, ИмяРеквизитаПуть = "ПутьСервер")
	...
	СтрокаВерсии[ИмяРеквизитаПуть] = ПараметрыOVM.ПолныйПутьККаталогуДистрибутивов();
